...
const RedisDB = require('../database/redis/redis');
 ...
 try {
    ...
    .post('/login', async (request, response, next) => {
        ...
        const [user] = await staffController.model.findOne({ 'email': `${email}` }, 'name role').exec();
        if (user) {
          await bcrypt.compare(password, user.password, function (err, result) {
            if (result) {
              ...
              RedisDB.client.set(`${token}`, `${user.id}-${user.role}`);
              RedisDB.client.expire(`${token}`, 86401);
     ...
     router
         ...
         .post('/logout', async (request, response, next) => {
